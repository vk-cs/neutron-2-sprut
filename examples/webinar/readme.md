...
**Важно!** После копирования ipsec, так как будет использоваться новый роутер с новым ip, необходимо на другой стороне туннеля поменять адрес пира (указать внешний ip продвинутого роутера). Для проверки туннеля не обязательно удалять туннель на нейтроне, можно удалить его после успешной проверки.
